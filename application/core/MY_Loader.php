			...
			foreach ($files as $file_array)
			{
				foreach ($file_array as $type => $file)
				{
					...
					if ($type == 'css')
					{
						...
						$includes .= link_tag(array('href' => $file,'rel' => 'stylesheet','type' => 'text/css','media' => 'screen'));
					}
					else if ($type == 'js')
					{
						//$includes .= '<script type="text/javascript" src="'.$file.'"></script>';
						$includes .= script_tag($file);
					}
				}
			}
	...
	{		
		...
		else
		{
			...
			foreach ($this->css_files as $css_file)
			{
				...
				$data['includes'] .= link_tag(array('href' => $css_file,'rel' => 'stylesheet','type' => 'text/css','media' => 'screen'));
			}
			...
			foreach ($this->js_files as $js_file)
			{
				...
				$data['includes'] .= script_tag($js_file);
			}

			return $this->_ci_load(array(
				'_ci_view' => '../../'.TEMPLATES_DIR.$this->template.'/theme.php', 
				'_ci_vars' => $data, 
				'_ci_return' => $return)
			);
		}
